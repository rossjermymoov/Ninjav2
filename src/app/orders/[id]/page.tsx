...
function getMockItems(orderId: string): OrderItem[] {
  ...
  return maps[orderId] ?? [
    { id: 'i1', product: 'Product', sku: (MOCK_ORDERS.find(o => o.id === orderId)?.sku[0] ?? 'SKU'), qty: 1, price: '£0.00' },
  ]
}

function getMockActivities(orderId: string): Activity[] {
  return [
...
// ─── Field row (pill style from SVG spec) ─────────────────────────────────────
// Left label: slight grey tint, rounded left; Right value: slight white tint, rounded right
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '0 14px',
        background: 'rgba(223,224,235,0.15)',
        borderRight: '1px solid rgba(253,255,255,0.08)',
      }}>
        <span style={{
          fontSize: '12px', fontWeight: font.weight.semibold,
          color: 'rgba(253,255,255,0.65)', fontFamily: M,
      ...
      {/* Value side */}
      <div style={{ flex: 1, height: '100%', display: 'flex', alignItems: 'center', padding: '0 14px' }}>
        {editable ? (
          <input
            value={value}
...
function CardTitle({ children }: { children: React.ReactNode }) {
  return (
    <span style={{
      fontSize: '11px', fontWeight: font.weight.extrabold,
      color: 'rgba(253,255,255,0.40)', fontFamily: font.family,
      textTransform: 'uppercase', letterSpacing: '0.10em', display: 'block',
      marginBottom: 14,
    }}>
      {children}
    </span>
...
function QtyBadge({ qty }: { qty: number }) {
  return (
    <div style={{
      ...
      <span style={{
        fontSize: '11px', fontWeight: font.weight.extrabold,
        color: '#0A0B1E', fontFamily: font.family,
      }}>
        {qty}
      </span>
    </div>
...
function ItemsTable({ items, onAddProduct }: { items: OrderItem[]; onAddProduct: () => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: 'grid',
        gridTemplateColumns: '1fr 130px 56px 80px 64px',
        background: '#171B2D',
        padding: '0 14px',
        height: 34,
        alignItems: 'center',
        ...
        {['Product', 'SKU', 'Qty', 'Price', ''].map((h, i) => (
          <span key={i} style={{
            ...
            color: 'rgba(253,255,255,0.50)', fontFamily: M,
      ...
      {items.map((item, idx) => (
        <div key={item.id}>
          <div style={{
            display: 'grid',
            gridTemplateColumns: '1fr 130px 56px 80px 64px',
            background: item.hasError ? 'rgba(205,28,105,0.15)' : 'rgba(253,255,255,0.05)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {item.hasError && (
                <div style={{
                  ...
                  <span style={{ fontSize: '11px', fontWeight: font.weight.extrabold, color: '#FDFFFF', fontFamily: M }}>!</span>
                </div>
              )}
              <span style={{
                fontSize: '13px', fontWeight: font.weight.semibold,
            ...
            <span style={{
              fontSize: '12px', fontWeight: font.weight.semibold,
              color: item.hasError ? '#FF6B8A' : 'rgba(253,255,255,0.60)', fontFamily: M,
              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            }}>
              {item.sku}
            </span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end' }}>
              {/* Edit */}
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, opacity: 0.8 }}
          ...
          {item.hasError && item.errorMsg && (
            <div style={{
              ...
              background: 'rgba(205,28,105,0.10)',
      ...
      <div style={{
        ...
        >
          ...
          <span style={{
            fontSize: '12px', fontWeight: font.weight.extrabold,
            color: '#0A0B1E', fontFamily: font.family,
          }}>
            Add product
          </span>
...
function ActivityEntry({ activity }: { activity: Activity }) {
  ...
  const dotColor = activity.type === 'error' ? colors.statusIssue
    ...
    : 'rgba(253,255,255,0.30)'

  return (
    <div style={{
      display: 'flex', gap: 10, paddingBottom: 14,
      borderBottom: '1px solid rgba(253,255,255,0.06)',
    }}>
      {/* Timeline dot */}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 3 }}>
          <span style={{
            fontSize: '11px', fontWeight: font.weight.bold,
            color: colors.mint, fontFamily: M,
          }}>
            {activity.timestamp}
          </span>
          <span style={{ fontSize: '11px', color: 'rgba(253,255,255,0.40)', fontFamily: M }}>
            {activity.user}
          </span>
        </div>
        <span style={{
          fontSize: '12px', fontWeight: font.weight.semibold,
          color: 'rgba(253,255,255,0.75)', fontFamily: M,
        }}>
          {activity.note}
        </span>
      </div>
...
export default function OrderDetailsPage() {
  ...
  const activities = getMockActivities(orderId)
  ...
  if (!order) {
    return (
      <div style={{
        ...
        color: colors.textMuted, fontFamily: M, fontSize: font.size.md,
      }}>
        Order not found.
      </div>
    )
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 style={{
              ...
              color: colors.textPrimary, fontFamily: M,
            }}>
              Order {order.orderNumber}
            </h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              <StatusBadge status={order.status} />
              <span style={{ fontSize: font.size.sm, color: colors.textSecondary, fontFamily: M }}>
                {order.createdAt}
              </span>
            </div>
          </div>
          ...
          <Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {order.externalOrderId && (
                <FieldRow label="Channel Ref"   value={order.externalOrderId} />
              )}
              <FieldRow label="Sales Channel"   value={order.channelStoreName} />
              <FieldRow label="Delivery Service" value={order.deliveryService} />
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 200 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 24 }}>
                  <span style={{ fontSize: '12px', color: 'rgba(253,255,255,0.50)', fontFamily: M }}>Subtotal</span>
                  <span style={{ fontSize: '13px', fontWeight: font.weight.semibold, color: '#FDFFFF', fontFamily: M }}>
                    £{subtotal.toFixed(2)}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 24 }}>
                  <span style={{ fontSize: '12px', color: 'rgba(253,255,255,0.50)', fontFamily: M }}>Shipping</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card style={{ padding: '16px 20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <span style={{ fontSize: '13px', fontWeight: font.weight.semibold, color: '#FDFFFF', fontFamily: M, display: 'block' }}>
                  {order.deliveryService}
                </span>
                <span style={{ fontSize: '12px', color: 'rgba(253,255,255,0.50)', fontFamily: M }}>
              ...
              >
                <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                  <path d="M4 10H2a1 1 0 01-1-1V5a1 1 0 011-1h12a1 1 0 011 1v4a1 1 0 01-1 1h-2M4 4V1h8v3M4 13h8v2H4z" stroke={colors.mint} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                Print Label
              </button>
            </div>
        ...
        <div>
          <Card style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ marginTop: 16 }}>
              <span style={{
                fontSize: '11px', fontWeight: font.weight.bold,
                color: 'rgba(253,255,255,0.40)', fontFamily: M,
                textTransform: 'uppercase', letterSpacing: '0.08em',
              ...
              <textarea
                ...
                onBlur={e => (e.currentTarget.style.borderColor = 'rgba(253,255,255,0.12)')}
              />
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                <button
                  ...
                  style={{
                    padding: '6px 16px', borderRadius: 99,
                    background: activityNote.trim() ? colors.mint : 'rgba(29,251,157,0.20)',
